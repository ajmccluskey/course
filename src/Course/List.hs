...
filter ::
  ...
  -> List a
filter f =
  foldRight (\a as -> bool as (a :. as) $ f a) Nil
...
seqOptional ::
  ...
  -> Optional (List a)
seqOptional =
  foldRight (\oa oas -> optional (\a -> mapOptional (a :.) oas) Empty oa) (Full Nil)
...
find ::
  ...
  -> Optional a
find f =
  foldRight (\a oa -> bool oa (Full a) $ f a) Empty
...
lengthGT ::
  ...
  -> Bool
lengthGT n =
  optional (const True) False . find ((== n) . fst) . zip infinity
