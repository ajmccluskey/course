...
(|||) ::
  ...
  -> Parser a
(|||) p1 p2 =
  P $ \i -> let pr = parse p1 i in bool pr (parse p2 i) . isErrorResult $ pr
...
instance Monad Parser where
  (=<<) ::
    ...
    -> Parser b
  (=<<) f p =
    P $ \i -> onResult (parse p i) (\i' a -> parse (f a) i')
...
satisfy ::
  ...
  -> Parser Char
satisfy f =
  character >>= lift3 bool unexpectedCharParser valueParser f
