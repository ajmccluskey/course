...
asMaybeZipper ::
  (ListZipper a -> MaybeListZipper b)
  -> MaybeListZipper a
  -> MaybeListZipper b
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper ls a rs) =
...
findLeft p (ListZipper ls a rs) =
  let
    (bLs, bRs) = break p ls
    newRs = foldLeft (flip (:.)) (a :. rs) bLs
  in
    case bRs of
      Nil -> IsNotZ
      (found :. as) -> IsZ $ ListZipper as found newRs
...
findRight p (ListZipper ls a rs) =
  let
    (bLs, bRs) = break p rs
    newLs = foldLeft (flip (:.)) (a :. ls) bLs
  in
    case bRs of
      Nil -> IsNotZ
      (found :. as) -> IsZ $ ListZipper newLs found as
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop lz@(ListZipper ls a rs) =
  case (ls,rs) of
    (Nil,Nil) -> lz
    (Nil, _:._) -> let (a':.newLs) = foldLeft (flip (:.)) Nil (a:.rs) in ListZipper newLs a' Nil
    (a':.newLs,_) -> ListZipper newLs a' (a:.rs)
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop lz@(ListZipper ls a rs) =
  case (ls,rs) of
    (Nil, Nil) -> lz
    (_:._, Nil) -> let (a':.newRs) = foldLeft (flip (:.)) Nil (a:.ls) in ListZipper Nil a' newRs
    (_, a':.newRs) -> ListZipper (a:.ls) a' newRs
...
moveLeft (ListZipper ls a rs) =
  case ls of
    Nil -> IsNotZ
    (a':.newLs) -> IsZ $ ListZipper newLs a' (a :. rs)
...
moveRight (ListZipper ls a rs) =
  case rs of
    Nil -> IsNotZ
    (a':.newRs) -> IsZ $ ListZipper (a:.ls) a' newRs
...
deletePullLeft ::
  ...
  -> MaybeListZipper a
deletePullLeft lz =
  (\(ListZipper ls a rs) -> ListZipper ls a (drop 1 rs)) >$> moveLeft lz
...
deletePullRight ::
  ...
  -> MaybeListZipper a
deletePullRight lz =
  (\(ListZipper ls a rs) -> ListZipper (drop 1 ls) a rs) >$> moveRight lz
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft newA (ListZipper ls a rs) =
  ListZipper (a :. ls) newA rs
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper ls r rs) =
  ListZipper ls a (r:.rs)
...
-- /Tip:/ Use `zipWith`
  (<*>) (ListZipper lfs f rfs) (ListZipper ls a rs) =
    ListZipper (zipWith ($) lfs ls) (f a) (zipWith ($) rfs rs)
...
instance Extend ListZipper where
  (<<=) f lz =
    ListZipper (go moveLeft lz) (f lz) (go moveRight lz)
    where
      go mv lz' =
        maybeZ (\lz'' -> f lz'' :. go mv lz'') Nil $ mv lz'
...
instance Traversable ListZipper where
  traverse f (ListZipper ls a rs) =
    ListZipper <$> traverse f ls <*> f a <*> traverse f rs
...
instance Traversable MaybeListZipper where
  traverse f =
    maybeZ ((IsZ <$>) . traverse f) (pure IsNotZ)
