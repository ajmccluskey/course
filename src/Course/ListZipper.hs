...
findLeft p (ListZipper l x r) =
  let (newRights, l') = break p l
      r' = toList (ListZipper newRights x r)
  in
    case l' of
      Nil    -> IsNotZ
...
moveLeftN ::
  ...
  -> MaybeListZipper a
moveLeftN n lz =
  let foo _ IsNotZ = IsNotZ
      foo _ (IsZ lz') = moveLeft lz'
   in foldRight foo (IsZ lz) (listh [1..n])
