...
distinct' ::
  ...
  -> List a
distinct' =
  (`eval` S.empty) . filtering ((not <$>) . isInSet)
...
instance forall f. Applicative f => Applicative (OptionalT f) where
  pure =
    OptionalT . pure . Full
...
-- [Full 2,Full 3,Empty]
instance forall f. Monad f => Monad (OptionalT f) where
  (=<<) f (OptionalT fa) =
    let
      g (Full a) = f a
      g Empty    = OptionalT . pure $ Empty
    in
      OptionalT $ (runOptionalT . g) =<< fa
